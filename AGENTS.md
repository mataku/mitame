...
- A `v*` tag must match the version in `Cargo.toml`, and `CHANGELOG.md` must have a `## <version>` section, which becomes the release notes; `VERSION_NAME` in `adapters/android/gradle.properties` must be that version or its `-SNAPSHOT` (it stays a snapshot between Android releases and is set to the plain version only for the `android-v*` tag, then bumped to the next `-SNAPSHOT`). Bump the files and add the section together. `mitame_flutter` versions independently: `adapters/flutter/binary_version` names the binary release it bundles, and `publish-flutter.yml` fails when that release does not exist.
