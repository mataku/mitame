...
The adapter is [`mitame_flutter`](https://pub.dev/packages/mitame_flutter) on pub.dev; add it with `flutter pub add --dev mitame_flutter`. Newer than 0.1.0, it bundles the binary for macOS arm64 and Linux (x64, arm64) and runs it through `dart run mitame_flutter:mitame <command>`, so a Flutter project needs no separate install and `pubspec.lock` pins the binary together with the adapter. On other platforms, or to run a local build, set `MITAME_BINARY` to a binary's path and the launcher runs that one instead; with 0.1.0, install the binary as for Android and iOS.

`loadFonts: true` loads the fonts declared in the package and the SDK's Roboto, so goldens render real glyphs instead of the Ahem placeholder font. `fonts = "ahem"` under `[capture]` in `mitame.toml` (or `MITAME_FONTS=ahem` in the environment) skips font loading, which renders text as Ahem boxes and gives one baseline for macOS and Linux, as described in [CI](ci.md). Ahem glyphs are wider than real ones and can overflow tight layouts, and packages that load fonts themselves (alchemist does) are unaffected by the switch.
