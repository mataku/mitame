...
The adapter is [`mitame_flutter`](https://pub.dev/packages/mitame_flutter) on pub.dev; add it with `flutter pub add --dev mitame_flutter`. It bundles the binary for macOS arm64 and Linux (x64, arm64) and runs it through `dart run mitame_flutter:mitame <command>`, so a Flutter project needs no separate install and `pubspec.lock` pins the binary together with the adapter. On other platforms, or to run a local build, set `MITAME_BINARY` to a binary's path and the launcher runs that one instead.
